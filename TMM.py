...
def reflect_amp(rho, ang_of_inc, wavelength, n, l, n_cover, n_subst):
    ...
    for i in np.arange(n.size):
        m = np.matmul(m,np.array([[np.cos(phi[i]),-1j*np.divide(np.sin(phi[i]),gamma[i])],[-1j*np.multiply(np.sin(phi[i]),gamma[i]),np.cos(phi[i])]]))
...
def ellips(ang_of_inc, wavelength, n, l, n_cover, n_subst):
    ...
    m0 = np.eye(2)
    for i in np.arange(n.size):
        m0 = np.matmul(m0,np.array([[np.cos(phi[i]),-1j*np.divide(np.sin(phi[i]),gamma0[i])],[-1j*np.multiply(np.sin(phi[i]),gamma0[i]),np.cos(phi[i])]]))
    #reflection coefficient
    ...
    m1 = np.eye(2)
    for i in np.arange(n.size):
        m1 = np.matmul(m1,np.array([[np.cos(phi[i]),-1j*np.divide(np.sin(phi[i]),gamma1[i])],[-1j*np.multiply(np.sin(phi[i]),gamma1[i]),np.cos(phi[i])]]))
    #reflection coefficient
    ...
    delta = np.angle(-r1/r0)*(180/np.pi)
